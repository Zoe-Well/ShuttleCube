...
export function OtherIncomeForm({ onDone }: { onDone?: () => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    mutation.mutate({ category: String(data.get("category")), received_at: beijingDateTimeInputToIso(String(data.get("received_at"))), amount: Number(data.get("amount")), payer: String(data.get("payer")), payment_method: String(data.get("payment_method")), notes: String(data.get("notes") || "") || null });
  };
