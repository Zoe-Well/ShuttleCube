...
import { beijingDateTimeInputToIso, formatBeijingDateTime, formatBeijingTime, toBeijingDateTimeInput } from "@/lib/beijing-time";
  ...
  const generate = useMutation({
    mutationFn: () =>
      generateReplacementCandidates(item, {
        window_start: beijingDateTimeInputToIso(windowStart),
        window_end: beijingDateTimeInputToIso(windowEnd),
        max_candidates: 20,
      }),
    ...
    <section className="panel p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">系统会在原教练和原场地可用时段中查找补排时间；学员是否方便仍需人工确认。</p>
        </div>
      </div>
      ...
      {!visibleApproval && !["resolved", "dismissed"].includes(item.state) ? (
        <>
          ...
          <button className="btn mt-3" disabled={generate.isPending || !windowStart || !windowEnd} onClick={() => generate.mutate()}>
            ...
            {generate.isPending ? "正在查找可用时间…" : "查找可用补排时间"}
          </button>
        </>
      ) : null}
            ...
            <label className={`block cursor-pointer rounded-xl border p-4 ${selectedPlanId === plan.resource_plan_id ? "border-emerald-500 bg-emerald-50" : "bg-white"}`} key={plan.resource_plan_id}>
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-medium">{formatBeijingDateTime(plan.starts_at)} 至 {formatBeijingTime(plan.ends_at)}</div>
                  <div className="mt-1 text-xs text-slate-500">原教练 {plan.coach_ids.length} 名 · 原场地 {plan.court_ids.length} 片 · 有效至 {formatBeijingDateTime(plan.expires_at)}</div>
                  {plan.ranking_explanation ? <p className="mt-2 text-xs text-slate-600">{plan.ranking_explanation}</p> : null}
                </div>
              </div>
            </label>
          )) : (
            <div className="rounded-lg bg-slate-50 p-4 text-sm text-slate-600">所选时间范围内没有可用补排时间，请扩大范围或回到人工排课页面处理。</div>
          )}
